...
func auditToolDefs() []toolDef {
	...
	return []toolDef{
		{Name: "asset_scan", Description: "构建小程序接口资产清单：从反编译代码目录（dir）、最近 500 条流量记录（includeTraffic 默认 true）与云函数名列表（cloudFns）三类来源提取 http(s)/ws 端点并去重成稳定资产（同一端点跨次运行同 ID）。appid 把流量收窄到单个小程序（清单是单个小程序的档案，建议传；缺省读全部程序的最近记录）。三类来源全空回 {ok:false,error:\"没有可用来源\"}；代码目录不可读是错误，即使流量记录存在。异步受理：回答 taskId 即返回，完成判据是 asset_list 看到 total 变化（进度事件 MCP 收不到）。只读本地代码与流量库，不对外发请求；dir 可从 code_projects 取",
			InputSchema: schema(map[string]any{
				"dir": str, "appid": str, "includeTraffic": boolean, "cloudFns": cloudFns,
			})},
		{Name: "asset_list", Description: "分页读取资产清单：kind（api/static/ws/cloud）与 host 精确匹配、query 按 URL 子串筛选；hosts 汇总随筛选联动（host + count，按命中数降序）。尚未构建过时回答空页（total:0、空 items/hosts）而不是报错——先用 asset_scan 构建。每条资产含 url/host/path/method、来源列表（code 文件或 traffic 记录 ID）、命中次数、首末观测时间与 trafficSeen",
			InputSchema: schema(map[string]any{
