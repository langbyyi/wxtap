...
// traffic_appids 让 agent 能枚举库里的小程序（appid + 记录数 + 已知名字）：
// traffic_list 的 appId 与 asset_scan 的 appid 都靠它来选，不该靠猜。转发必须
// 直通 traffic.appids 且不带任何参数，并标注只读。
func TestTrafficAppIDsToolForwardsToTrafficAppIDs(t *testing.T) {
	bridge := &paramBridge{}
	messages := exchange(t, Deps{AppBridge: bridge},
		`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"traffic_appids","arguments":{}}}`+"\n")
	if text := toolText(t, messages, 1); strings.Contains(text, "unknown tool") {
		t.Fatalf("traffic_appids 未注册：%s", text)
	}
	if len(bridge.calls) != 1 {
		t.Fatalf("forwards = %d, want 1", len(bridge.calls))
	}
	if bridge.calls[0].method != "traffic.appids" {
		t.Fatalf("forwarded method = %q, want traffic.appids", bridge.calls[0].method)
	}
	if len(bridge.calls[0].args) != 0 {
		t.Fatalf("forwarded args = %v, want none", bridge.calls[0].args)
	}
	for _, tool := range New(Deps{ToolProfile: "all"}).tools() {
		if tool.Name != "traffic_appids" {
			continue
		}
		if !tool.Annotations.ReadOnly {
			t.Fatal("traffic_appids 必须标注只读")
		}
		return
	}
	t.Fatal("tools/list 没有列出 traffic_appids")
}
