...
// decompiledDir resolves the one decompile-output directory an appid can have
// (<dataDir>/output/<appid> — the same root the extract page and the settings
// page show as 反编译输出目录). Empty when there is no config base or the
// directory does not exist, so the build falls back to the remaining sources.
func (s *AuditService) decompiledDir(appid string) string {
	appid = strings.TrimSpace(appid)
	if appid == "" || s.config == nil {
		return ""
	}
	dir := filepath.Join(s.config.Dir(), "output", appid)
...
func (s *AuditService) AssetsScan(params AssetsScanParams) (map[string]any, error) {
	// 目录不再由界面手填：一个 appid 的产物目录固定在 output/<appid>，缺省
	// dir 时按目标自动解析（未反编译则保持空，构建退回其余来源）。
	if strings.TrimSpace(params.Dir) == "" {
		params.Dir = s.decompiledDir(params.AppID)
	}
