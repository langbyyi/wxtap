...
async function browserMock<T>(method: string, params?: Record<string, unknown>): Promise<T> {
  ...
  if (method === 'traffic.appids') {
    ...
    // 连接时记住的小程序名（后端从 config 注册表回填）。
    return [{ appid: 'wxdeadbeefdeadbeef', name: '预览示例小程序', count: 5, lastSeen: new Date().toISOString() }] as T;
  }
  ...
  if (method === 'traffic.list') {
    ...
    // appId 精确过滤与真实后端一致：历史记录按小程序切换时预览也要筛得动。
    const filtering = params as { appId?: string } | undefined;
    const scoped = filtering?.appId ? items.filter((item) => item.appId === filtering.appId) : items;
    ...
    return { items: scoped.slice((page - 1) * pageSize, page * pageSize), total: filtering?.appId ? scoped.length : 137, page, pageSize } as T;
  }
  ...
  // 反编译：预览里瞬间完成。资产页的「反编译当前小程序」按钮据此走通
  // 反编译 → 刷新产物列表 → 自动选中的链路。
  if (method === 'extract.decompile') {
    return { ok: true, files_count: 42, name: '预览示例小程序', dir: PREVIEW_ROOT, output_dir: PREVIEW_ROOT, packages_processed: 2 } as T;
  }
