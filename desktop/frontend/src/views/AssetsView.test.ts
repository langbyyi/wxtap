...
describe('AssetsView', () => {
  ...
  it('restores the last target from config and shows the built-at hint', async () => {
    ...
    expect(wrapper.get('[data-testid="assets-built-hint"]').text()).toContain('wx-last');
  ...
  it('aligns the decompile dir to the connected program by appid, never by recency', async () => {
    // 两个产物：wx-mine 是连接中的目标但 mtime 较旧；wx-other 更新。
    // 连接后目录必须对齐 wx-mine 自己的产物——不是最近的那份。
    ...
    expect(wrapper.get<HTMLSelectElement>('[data-testid="assets-target"]').element.value).toBe('wx-mine');
    expect(wrapper.get<HTMLInputElement>('[data-testid="assets-dir"]').element.value).toBe('C:/out/wx-mine');

    // 构建参数同时带目标 appid 与对齐后的目录：流量、代码同口径
    await wrapper.get('[data-testid="assets-build"]').trigger('click');
    await flushPromises();
    expect(call).toHaveBeenCalledWith('assets.scan', expect.objectContaining({ appid: 'wx-mine', dir: 'C:/out/wx-mine' }));
  });

  it('clears a stale decompile dir when the target has no output of its own', async () => {
    // 先连 wx-a（有产物，目录对齐过去）；再切到 wx-b（没有任何产物）：
    // 残留的 wx-a 目录必须清掉——不清洗会把别家代码扫进 wx-b 的清单。
    call.mockImplementation((method: string) => {
      if (method === 'code.projects') {
        return Promise.resolve({ projects: [{ appid: 'wx-a', path: 'C:/out/wx-a', mtime: 100 }] });
      }
      if (method === 'assets.list') return Promise.resolve({ ok: true, total: 0, hosts: [], items: [] });
      return Promise.resolve({ ok: true });
    });
    engineHolder.state!.status.appInfo = { appid: 'wx-a', name: 'A' };
    const wrapper = mount(AssetsView);
    await flushPromises();
    expect(wrapper.get<HTMLInputElement>('[data-testid="assets-dir"]').element.value).toBe('C:/out/wx-a');
    ...
    expect(wrapper.get<HTMLInputElement>('[data-testid="assets-dir"]').element.value).toBe('');
  });
  ...
  it('builds the inventory from the form values and reports completion', async () => {    const wrapper = mount(AssetsView);
    ...
    await wrapper.get('[data-testid="assets-dir"]').setValue('C:/wxtap/wxdeadbeef');
    await wrapper.get('[data-testid="assets-cloud-fns"]').setValue('login\n getOrder \n\n');
    await wrapper.get('[data-testid="assets-build"]').trigger('click');
    await flushPromises();
    // 每行一个云函数名，去空白；空行丢弃
    expect(call).toHaveBeenCalledWith('assets.scan', { dir: 'C:/wxtap/wxdeadbeef', includeTraffic: true, cloudFns: ['login', 'getOrder'] });
  ...
  it('builds without a directory when only traffic is included', async () => {
    const wrapper = mount(AssetsView);
    await flushPromises();
    await wrapper.get('[data-testid="assets-include-traffic"]').setValue(false);
    await wrapper.get('[data-testid="assets-build"]').trigger('click');
    await flushPromises();
    expect(call).toHaveBeenCalledWith('assets.scan', { includeTraffic: false });
  });
