...
// ---- 目标小程序 ----
// 清单是单个小程序的档案：一个小程序只有一个反编译产物目录（后端按
// output/<appid> 解析，与设置页的「反编译输出目录」同源），目录不再手填。
// 下拉只列已反编译的小程序，目标默认跟随当前连接；断开后保持上次选择
// （config 持久化）。连接中的程序还没反编译时给一键反编译入口。
type AppIDOption = { appid: string; name?: string; connected?: boolean };
...
const connectedAppID = computed(() => engine.status.appInfo?.appid || '');
const appOptions = computed<AppIDOption[]>(() =>
  knownProjects.value
    .filter((project) => !!project.appid)
    .map((project) => ({ appid: project.appid!, name: project.name, connected: project.appid === connectedAppID.value }))
    .sort((a, b) => Number(!!b.connected) - Number(!!a.connected) || a.appid.localeCompare(b.appid)),
);
// 连接中的小程序还没有产物：露出一键反编译入口，完成后它会进入下拉。
const connectedDecompiled = computed(() => !!connectedAppID.value && appOptions.value.some((option) => option.appid === connectedAppID.value));
...
async function startBuild() {
  if (building.value || !targetAppID.value) return;
  buildError.value = '';
  buildNotice.value = '';
  // 清单按目标小程序构建：流量只取它的记录；代码来源由后端按 appid 自动
  // 解析 output/<appid>（与反编译页、设置页同一目录），界面不再传 dir。
  const params: Record<string, unknown> = { appid: targetAppID.value, includeTraffic: includeTraffic.value };
...
// 目标与云函数名写入 config.json：重启后据此恢复上次的目标与存档。
...
async function refreshProjects() {
  const res = await backend.call<{ projects?: Array<{ appid?: string; name?: string; path?: string; mtime?: number }> }>('code.projects');
  if (!disposed) knownProjects.value = res?.projects ?? [];
}

// 下拉只列已反编译的产物（code.projects，与反编译页同一目录口径）。
// 目标优先级：当前连接（已反编译才可选）> 上次保存（仍在列表）> 空（请选择）。
async function bootstrapTarget() {
  const [configRes, projectsRes] = await Promise.allSettled([
  ...
  const connected = connectedAppID.value;
  if (appOptions.value.some((option) => option.appid === connected)) targetAppID.value = connected;
  else if (appOptions.value.some((option) => option.appid === savedAppID)) targetAppID.value = savedAppID;
...
// 连接中的小程序还没有产物：直接反编译（复用反编译页的 extract.decompile，
// 包目录与输出目录都和它同源），完成后刷新列表并选中它。
async function decompileConnected() {
  const appid = connectedAppID.value;
  if (!appid || decompiling.value) return;
  decompiling.value = true;
  buildError.value = '';
  try {
    await backend.call('extract.decompile', { appid });
    if (disposed) return;
    await refreshProjects();
    if (disposed) return;
    targetAppID.value = appid;
    notify('反编译完成，已选为清单目标', 'success');
  } catch (reason) {
    if (!disposed) {
      buildError.value = `反编译失败：${messageOf(reason)}`;
      notify(`反编译失败：${messageOf(reason)}`, 'error');
    }
  } finally {
    if (!disposed) decompiling.value = false;
  }
}
...
// 连接身份变化即跟随：只跟随已反编译的小程序。未反编译的连接不切换目标，
// 走「反编译当前小程序」，完成后自动进入下拉并选中。
watch(connectedAppID, (appid) => {
  if (appid && appOptions.value.some((option) => option.appid === appid)) targetAppID.value = appid;
});
    ...
    <div class="panel build-panel">
      <div class="panel-body">
        <div class="build-grid">
          <label class="field inline-field grow-target" for="assets-target"><span>目标小程序</span>
            <select id="assets-target" v-model="targetAppID" data-testid="assets-target" title="清单按该小程序构建：流量只取它的记录，代码取它的反编译产物（目录自动对应，无需手填）；当前连接的小程序会自动选中">
              <option value="" disabled>请选择小程序</option>
              <option v-for="option in appOptions" :key="option.appid" :value="option.appid">{{ optionLabel(option) }}</option>
            </select>
          </label>
          <button v-if="connectedAppID && !connectedDecompiled" data-testid="assets-decompile-connected" type="button" class="ghost" :disabled="decompiling" @click="decompileConnected" title="把当前连接的小程序反编译到统一输出目录，完成后自动选为清单目标">{{ decompiling ? '反编译中…' : '反编译当前小程序' }}</button>
          ...
          <button data-testid="assets-build" type="button" :disabled="building || !targetAppID" @click="startBuild">{{ building ? '构建中…' : '构建清单' }}</button>
        </div>
        <p v-if="builtHint || buildNotice" data-testid="assets-built-hint" class="status-line built-hint">
          <template v-if="builtHint">{{ builtHint }}</template>
          <span v-if="builtHint && buildNotice" aria-hidden="true" class="hint-sep">·</span>
          <span v-if="buildNotice" class="notice-text" data-testid="assets-build-notice" role="status">{{ buildNotice }}</span>
        </p>
    ...
    <div class="panel assets-panel">
      ...
      <template v-if="viewMode !== 'table'">
        ...
        <EmptyState
          ...
          title="还没有资产"
          description="先选择目标小程序并点击上方「构建清单」，从反编译产物与抓包流量构建；或放宽筛选条件后再试。"
        />
      </template>
