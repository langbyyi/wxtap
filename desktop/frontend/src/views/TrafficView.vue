...
// 小程序下拉的选项：库里出现过的 appid + 记录数（traffic.appids）。加载失败
// 只是少一个快捷入口，AppID 手输框仍然可用。
type AppIDStat = { appid: string; count: number; lastSeen?: string };
      ...
      <header class="panel-header traffic-filters">
        ...
        <div class="toolbar filter-tools">
          ...
          <select v-if="appIdOptions.length" :value="appIdFilter" data-testid="traffic-appid-picker" aria-label="按小程序筛选" @change="pickAppId">
            ...
            <option v-for="stat in appIdOptions" :key="stat.appid" :value="stat.appid" :title="`该小程序共 ${stat.count} 条记录`">{{ stat.appid }}（{{ stat.count }}）</option>
          </select>
